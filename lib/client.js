...
class Client {
  ...
  _convertToSnakeCase(payload) {
    const snakeCasePayload = _.mapKeys(payload, function(value, key) {
      let returnKey = _.snakeCase(key);
      const lastCharacter = returnKey.charAt(returnKey.length-1);
      if(lastCharacter >= '0' && lastCharacter <= '9') {
        returnKey = returnKey.substring(0, returnKey.length-2);
        returnKey += lastCharacter;
      }
      return returnKey;
    });
   ...
   * @returns {Promise}
   */
  _getParamsAndUrl(_this, queryParams, methodName) {
    return new Promise(function (resolve, reject) {
      try {
        queryParams = _this._convertToSnakeCase(queryParams);
        const url = _this._getUrl(methodName, {}, queryParams);
  ...
  getStudentReservationList(queryParams) {
    ...
    return this
      ...
      .then(()=>{
        ...
        return this._getParamsAndUrl(this, queryParams, methodName);
      })
      .then((response) => {
   ...
   * @returns {Promise}
   */
  _postPayloadAndUrl(_this, payload, methodName) {
    return new Promise(function (resolve, reject) {
      try{
        payload = _this._convertToSnakeCase(payload);
        const url = _this._getUrl(methodName);
  ...
  beginReservation(payload) {
    ...
    return this
      ...
      .then(()=>{
        //convert params to snakeCase and make url
        return this._postPayloadAndUrl(this, payload, methodName);
      })
      .then((response) => {
        return this
          ._post(response.url)
