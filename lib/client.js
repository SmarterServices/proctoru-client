...
class Client {
  ...
  removeReservation(options) {
    ...
    if(!options.explanation)
      options.explanation = 'Cancelled via SmarterServices API.'
  ...
  /**
   * Creates an institutional user
   * @params {Object} options - The payload to request with
   * @param {string} options.host - Host name for request
   * @param {string} options.authorizationToken - Api Token to send in header
   * @param {string} [options.userPassword] - User password for institutional user
   * @params {string} options.lastName - Institutional user's last name
   * @params {string} options.firstName - Institutional user's first name
   * @params {string} options.address1 - Institutional user's Address 1 *required if US only
   * @params {string} options.city - Institutional user's City *required if US only
   * @params {string} options.state - Institutional user's State
   * @params {string} options.country - Institutional user's Country
   * @params {string} options.zipcode - Institutional user's zip or postal code
   * @params {string} options.phone1 - Institutional user's Primary Telephone number
   * @params {string} options.email - Institutional user's Email
   * @params {string} options.timeZoneId - Institutional user's timeZone Id
   * @params {string} options.role - Institutional user's role
   * @returns {Promise} - User institution data
   */
  editInstitutionalUser(options) {
    const methodName = 'editInstitutionalUser';
   ...
   */
  getInstitutionDepartmentList(options) {
    const methodName = 'getInstitutionDepartmentList';

    return this
      ._validateOptions(methodName, options)
      .then(({ credentials }) => this._getParamsAndUrl({}, credentials, methodName))
      .then(({ url, method, authorizationToken }) =>
        this._sendRequest(url, method, authorizationToken))
      .then((response) => camelize(response.data))
      .catch((error) => Promise.reject(camelize(error)));
  }
}
